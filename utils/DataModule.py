...
class DataModule():
    ...
    def prepareModelSupplement(self, model):
        ...
        # MODIFIED: GraphTrust 物品-物品图 (被同一用户消费过的物品互相关联)
        if 'ITEM_ITEM_SPARSE_MATRIX' in model.supply_set:
            self.generateItemItemSparseMatrix()
            data_dict['ITEM_ITEM_INDICES_INPUT'] = self.item_item_indices_list
            data_dict['ITEM_ITEM_VALUES_INPUT'] = self.item_item_values_list
    ...
    def readData(self):
        ...
        # 3\t105
        # 结果:
        # hash_data = {(1,100): 1, (1,102): 1, (2,100): 1, (3,105): 1}
        # total_user_list = [1, 2, 3]
    ...
    def arrangePositiveData(self):
        ...
        # 示例
        # 输入 hash_data: 
        # {(1,100): 1, (1,102): 1, (2,100): 1, (3,105): 1}

        # 输出 positive_data:
        # {
        # 1: {100, 102},      # 用户1交互了项目100和102
        # 2: {100},           # 用户2交互了项目100
        # 3: {105}            # 用户3交互了项目105
        # }
    ...
    def arrangePositiveDataForItemUser(self):
        ...
        # positive_data_for_item_user:
        # {
        # 100: {1, 2},    # 项目100被用户1和2交互过
        # 102: {1},       # 项目102被用户1交互过
        # 105: {3}        # 项目105被用户3交互过
        # }
    ...
    # 随机采样过程:
    # 第1个负样本: 采到105 ✓ (不在正样本中)
    # 第2个负样本: 采到100 ✗ (在正样本中，重新采)
    #         采到108 ✓ (不在正样本中)

    # negative_data[1] = {105, 108}
    ...
    # 执行过程:
    # 循环 1: (1,100) → user_list=[1], item_list=[100], index_dict[1]=[0], index=1
    # 循环 2: (1,102) → user_list=[1,1], item_list=[100,102], index_dict[1]=[0,1], index=2
    # 循环 3: (2,100) → user_list=[1,1,2], item_list=[100,102,100], index_dict[2]=[2], index=3
    # 循环 4: (3,105) → user_list=[1,1,2,3], item_list=[100,102,100,105], index_dict[3]=[3], index=4

    # 最终结果:
    # eva_user_list = [[1], [1], [2], [3]]              # 形状: (4, 1)
    # eva_item_list = [[100], [102], [100], [105]]      # 形状: (4, 1)
    # eva_index_dict = {
    #     1: [0, 1],       # 用户1的正样本在列表中的位置是0和1
    #     2: [2],          # 用户2的正样本在列表中的位置是2
    #     3: [3]           # 用户3的正样本在列表中的位置是3
    # }
    ...
    # MODIFIED: GraphTrust 物品-物品图构建
    # 被同一用户消费过的物品之间建立关联边 (关系4)
    def generateItemItemSparseMatrix(self):
        positive_data = self.positive_data
        item_item_dict = defaultdict(set)

        for u in positive_data:
            items = list(positive_data[u])
            for idx, i in enumerate(items):
                for j in items[idx+1:]:
                    item_item_dict[i].add(j)
                    item_item_dict[j].add(i)

        item_item_indices_list = []
        item_item_values_list = []

        for item in range(self.conf.num_items):
            if item in item_item_dict:
                for related_item in item_item_dict[item]:
                    item_item_indices_list.append([item, related_item])
                    item_item_values_list.append(1.0/len(item_item_dict[item]))

        self.item_item_indices_list = np.array(item_item_indices_list).astype(np.int64) if item_item_indices_list else np.empty((0, 2), dtype=np.int64)
        self.item_item_values_list = np.array(item_item_values_list).astype(np.float32) if item_item_values_list else np.empty((0,), dtype=np.float32)
